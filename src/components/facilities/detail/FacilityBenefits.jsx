...
export default function FacilityBenefits({ facility }) {
  return (
      ...
      <Container className="relative grid gap-12 lg:grid-cols-12 lg:items-center lg:gap-16">
        <div className="lg:col-span-5">
          <SectionHeading
            tone="light"
            eyebrow="Benefits"
            title={`Why our ${facility.title} makes a difference`}
            description="Having this facility inside the hospital means faster decisions, fewer delays and less running around for families."
          />
        ...
        <ul className="grid gap-5 sm:grid-cols-2 lg:col-span-7">
          {facility.benefits.map(({ icon: Icon, title, description }, index) => (
            ...
            >
              ...
              <span className="grid h-12 w-12 place-items-center rounded-2xl bg-linear-to-br from-accent to-primary text-white shadow-lg shadow-black/20">
